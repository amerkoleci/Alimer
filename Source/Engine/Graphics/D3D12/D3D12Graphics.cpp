...
#pragma comment(lib,"dxgi.lib")
        ...
        : minFeatureLevel(minFeatureLevel_)
    {
...

        {
            ...
            HRESULT hr = D3D12CreateDevice(adapter.Get(), minFeatureLevel, IID_PPV_ARGS(&device));
            ThrowIfFailed(hr);
    ...
    {
        size = AlignTo(size, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
    ...
    ShaderRef D3D12Graphics::CreateShader(ShaderStage stage, const std::vector<uint8_t>& byteCode, const std::string& entryPoint)
    {
    ...
    SwapChainRef D3D12Graphics::CreateSwapChain(void* windowHandle, const SwapChainCreateInfo& info)
    {
        ...
        if (result->GetHandle() != nullptr)
        {
            return RefPtr<SwapChain>(result);
        }
