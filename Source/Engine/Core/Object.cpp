...
namespace Alimer
{
	namespace details
	{
		struct Context
		{
			...
			SharedPtr<Object> CreateObject(StringId32 type, const StringView& name)
			{
				auto it = factories.find(type);
				return it != factories.end() ? it->second->Create(name) : nullptr;
			}

			const String& GetTypeNameFromType(StringId32 type)
			{
    ...
    SharedPtr<Object> Object::CreateObject(StringId32 objectType, const StringView& name)
	{
		return details::context().CreateObject(objectType, name);
	}

	const String& Object::GetTypeNameFromType(StringId32 type)
	{
