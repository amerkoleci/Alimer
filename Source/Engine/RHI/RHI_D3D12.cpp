...
#pragma comment(lib,"dxgi.lib")
        ...
        struct SingleDescriptor
        {
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_CONSTANT_BUFFER_VIEW_DESC& cbv)
            {
                this->cbv = cbv;
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_SHADER_RESOURCE_VIEW_DESC& srv, ID3D12Resource* res)
            {
                this->srv = srv;
                this->allocationhandler = device->allocationhandler;
                type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_UNORDERED_ACCESS_VIEW_DESC& uav, ID3D12Resource* res)
            {
                this->uav = uav;
                this->allocationhandler = device->allocationhandler;
                type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_SAMPLER_DESC& sam)
            {
                this->sam = sam;
                this->allocationhandler = device->allocationhandler;
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_RENDER_TARGET_VIEW_DESC& rtv, ID3D12Resource* res)
            {
                this->rtv = rtv;
                this->allocationhandler = device->allocationhandler;
            ...
            void init(const RHIDeviceD3D12* device, const D3D12_DEPTH_STENCIL_VIEW_DESC& dsv, ID3D12Resource* res)
            {
                this->dsv = dsv;
                this->allocationhandler = device->allocationhandler;
        ...
        struct RenderPass_DX12
        {
            ...
            const RHITexture* shading_rate_image = nullptr;
        ...
        Texture_DX12* to_internal(const RHITexture* param)
        {
    ...
    namespace
    {
        static_assert(sizeof(Viewport) == sizeof(D3D12_VIEWPORT), "Size mismatch");
        static_assert(offsetof(Viewport, x) == offsetof(D3D12_VIEWPORT, TopLeftX), "Layout mismatch");
        static_assert(offsetof(Viewport, y) == offsetof(D3D12_VIEWPORT, TopLeftY), "Layout mismatch");
        static_assert(offsetof(Viewport, width) == offsetof(D3D12_VIEWPORT, Width), "Layout mismatch");
        static_assert(offsetof(Viewport, height) == offsetof(D3D12_VIEWPORT, Height), "Layout mismatch");
        static_assert(offsetof(Viewport, minDepth) == offsetof(D3D12_VIEWPORT, MinDepth), "Layout mismatch");
        static_assert(offsetof(Viewport, maxDepth) == offsetof(D3D12_VIEWPORT, MaxDepth), "Layout mismatch");
    ...
    void RHIDeviceD3D12::FrameResources::ResourceFrameAllocator::init(RHIDeviceD3D12* device, size_t size)
    {
        this->device = device;
    ...
    void RHIDeviceD3D12::FrameResources::ResourceFrameAllocator::clear()
    {
        dataCur = dataBegin;
    }
    uint64_t RHIDeviceD3D12::FrameResources::ResourceFrameAllocator::calculateOffset(uint8_t* address)
    {
        assert(address >= dataBegin && address < dataEnd);
        return static_cast<uint64_t>(address - dataBegin);
    }

    void RHIDeviceD3D12::DescriptorBinder::init(RHIDeviceD3D12* device)
    {
        this->device = device;
    ...
    void RHIDeviceD3D12::DescriptorBinder::reset()
    {
        dirty_res = true;
        dirty_sam = true;
        ringOffset_res = 0;
    ...
    void RHIDeviceD3D12::DescriptorBinder::flush(bool graphics, CommandList cmd)
    {
    ...
    void RHIDeviceD3D12::pso_validate(CommandList cmd)
    {
        if (!dirty_pso[cmd])
            return;
    ...
    void RHIDeviceD3D12::query_flush(CommandList cmd)
    {
    ...
    void RHIDeviceD3D12::barrier_flush(CommandList cmd)
    {
        ...
        if (!barriers.empty())
        {
            for (size_t i = 0; i < barriers.size(); ++i)
            {
                ...
                if (barrier.Type == D3D12_RESOURCE_BARRIER_TYPE_TRANSITION &&
                    cmd_meta[cmd].queue != RHIQueueType::Graphics)
                {
    ...
    void RHIDeviceD3D12::predraw(CommandList cmd)
    {
        pso_validate(cmd);
    ...
    void RHIDeviceD3D12::predispatch(CommandList cmd)
    {
        barrier_flush(cmd);
    ...
    bool RHIDeviceD3D12::Initialize()
    {
        ...
        queues[(uint32_t)RHIQueueType::Graphics].desc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
        queues[(uint32_t)RHIQueueType::Graphics].desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL;
        queues[(uint32_t)RHIQueueType::Graphics].desc.Flags = D3D12_COMMAND_QUEUE_FLAG_NONE;
        queues[(uint32_t)RHIQueueType::Graphics].desc.NodeMask = 0;
        hr = device->CreateCommandQueue(&queues[(uint32_t)RHIQueueType::Graphics].desc, IID_PPV_ARGS(&queues[(uint32_t)RHIQueueType::Graphics].queue));
        assert(SUCCEEDED(hr));
        hr = device->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(&queues[(uint32_t)RHIQueueType::Graphics].fence));
        assert(SUCCEEDED(hr));

        queues[(uint32_t)RHIQueueType::Compute].desc.Type = D3D12_COMMAND_LIST_TYPE_COMPUTE;
        queues[(uint32_t)RHIQueueType::Compute].desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL;
        queues[(uint32_t)RHIQueueType::Compute].desc.Flags = D3D12_COMMAND_QUEUE_FLAG_NONE;
        queues[(uint32_t)RHIQueueType::Compute].desc.NodeMask = 0;
        hr = device->CreateCommandQueue(&queues[(uint32_t)RHIQueueType::Compute].desc, IID_PPV_ARGS(&queues[(uint32_t)RHIQueueType::Compute].queue));
        assert(SUCCEEDED(hr));
        hr = device->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(&queues[(uint32_t)RHIQueueType::Compute].fence));
        assert(SUCCEEDED(hr));
        ...
        {
            LOGF("DX12: Root signature version 1.1 not supported!");
        }
    ...
    bool RHIDeviceD3D12::CreateBuffer(const BufferDescriptor* descriptor, const void* initialData, GPUBuffer* pBuffer) const
    {
        ...
        device->GetCopyableFootprints(&desc, 0, 1, 0, &internal_state->footprint, nullptr, nullptr, &pBuffer->allocatedSize);

        HRESULT result = allocationhandler->allocator->CreateResource(
            &allocationDesc,
            &desc,
            resourceState,
            nullptr,
    ...
    bool RHIDeviceD3D12::CreateTexture(const TextureDesc* pDesc, const SubresourceData* pInitialData, RHITexture* pTexture) const
    {
    ...
    bool RHIDeviceD3D12::CreateShader(ShaderStage stage, const void* pShaderBytecode, size_t BytecodeLength, Shader* pShader) const
    {
...

        {
            ...
            if (stage == ShaderStage::Compute || stage == ShaderStage::Library)
            {
                ...
                HashCombine(rootconstant_hash, internal_state->rootconstants.ShaderVisibility);
                HashCombine(rootconstant_hash, internal_state->rootconstants.ParameterType);
                HashCombine(rootconstant_hash, internal_state->rootconstants.Constants.Num32BitValues);
                HashCombine(rootconstant_hash, internal_state->rootconstants.Constants.RegisterSpace);
                HashCombine(rootconstant_hash, internal_state->rootconstants.Constants.ShaderRegister);

                size_t root_binding_hash = 0;
                for (auto& x : internal_state->root_cbvs)
                {
                    HashCombine(root_binding_hash, x.Flags);
                    HashCombine(root_binding_hash, x.ShaderRegister);
                    HashCombine(root_binding_hash, x.RegisterSpace);
                }

                size_t resource_binding_hash = 0;
                for (auto& x : internal_state->resources)
                {
                    HashCombine(resource_binding_hash, x.BaseShaderRegister);
                    HashCombine(resource_binding_hash, x.NumDescriptors);
                    HashCombine(resource_binding_hash, x.Flags);
                    HashCombine(resource_binding_hash, x.OffsetInDescriptorsFromTableStart);
                    HashCombine(resource_binding_hash, x.RangeType);
                    HashCombine(resource_binding_hash, x.RegisterSpace);
                }

                size_t sampler_binding_hash = 0;
                for (auto& x : internal_state->samplers)
                {
                    HashCombine(sampler_binding_hash, x.BaseShaderRegister);
                    HashCombine(sampler_binding_hash, x.NumDescriptors);
                    HashCombine(sampler_binding_hash, x.Flags);
                    HashCombine(sampler_binding_hash, x.OffsetInDescriptorsFromTableStart);
                    HashCombine(sampler_binding_hash, x.RangeType);
                    HashCombine(sampler_binding_hash, x.RegisterSpace);
                }

                size_t bindless_hash = 0;
                for (auto& x : internal_state->bindless_res)
                {
                    HashCombine(bindless_hash, x.BaseShaderRegister);
                    HashCombine(bindless_hash, x.NumDescriptors);
                    HashCombine(bindless_hash, x.Flags);
                    HashCombine(bindless_hash, x.OffsetInDescriptorsFromTableStart);
                    HashCombine(bindless_hash, x.RangeType);
                    HashCombine(bindless_hash, x.RegisterSpace);
                }
                ...
                HashCombine(rootsig_hash, rootconstant_hash);
                HashCombine(rootsig_hash, root_binding_hash);
                HashCombine(rootsig_hash, resource_binding_hash);
                HashCombine(rootsig_hash, sampler_binding_hash);
                HashCombine(rootsig_hash, bindless_hash);
                for (auto& x : internal_state->staticsamplers)
                {
                    HashCombine(rootsig_hash, x.AddressU);
                    HashCombine(rootsig_hash, x.AddressV);
                    HashCombine(rootsig_hash, x.AddressW);
                    HashCombine(rootsig_hash, x.BorderColor);
                    HashCombine(rootsig_hash, x.ComparisonFunc);
                    HashCombine(rootsig_hash, x.Filter);
                    HashCombine(rootsig_hash, x.MaxAnisotropy);
                    HashCombine(rootsig_hash, x.MaxLOD);
                    HashCombine(rootsig_hash, x.MinLOD);
                    HashCombine(rootsig_hash, x.MipLODBias);
                    HashCombine(rootsig_hash, x.RegisterSpace);
                    HashCombine(rootsig_hash, x.ShaderRegister);
                    HashCombine(rootsig_hash, x.ShaderVisibility);
                }
    ...
    bool RHIDeviceD3D12::CreatePipelineState(const PipelineStateDesc* pDesc, PipelineState* pso) const
    {
        auto internal_state = std::make_shared<PipelineState_DX12>();
        internal_state->allocationhandler = allocationhandler;
...

        {
            ...
            size_t rootsig_hash = 0;
            HashCombine(rootsig_hash, pDesc->il);
            HashCombine(rootsig_hash, rootconstant_hash);
            HashCombine(rootsig_hash, root_binding_hash);
            HashCombine(rootsig_hash, resource_binding_hash);
            HashCombine(rootsig_hash, sampler_binding_hash);
    ...
    bool RHIDeviceD3D12::CreateRaytracingAccelerationStructure(const RaytracingAccelerationStructureDesc* pDesc, RaytracingAccelerationStructure* bvh) const
    {
            ...
            case RaytracingAccelerationStructureDesc::BOTTOMLEVEL:
            {
                ...
                for (auto& x : pDesc->bottomlevel.geometries)
                {
                    ...
                    if (x.type == RaytracingAccelerationStructureDesc::BottomLevel::Geometry::TRIANGLES)
                    {
                        ...
                        geometry.Triangles.IndexFormat = (x.triangles.indexFormat == IndexType::UInt16 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT);
                        geometry.Triangles.IndexBuffer = to_internal(&x.triangles.indexBuffer)->gpu_address +
                            (D3D12_GPU_VIRTUAL_ADDRESS)x.triangles.indexOffset * (x.triangles.indexFormat == IndexType::UInt16 ? sizeof(uint16_t) : sizeof(uint32_t));
    ...
    bool RHIDeviceD3D12::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* pDesc, RaytracingPipelineState* rtpso) const
    {
    ...
    int RHIDeviceD3D12::CreateSubresource(RHITexture* texture, SUBRESOURCE_TYPE type, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount) const
    {
    ...
    int RHIDeviceD3D12::GetDescriptorIndex(const GPUResource* resource, SUBRESOURCE_TYPE type, int subresource) const
    {
        if (resource == nullptr || !resource->IsValid())
            return -1;
    ...
    int RHIDeviceD3D12::GetDescriptorIndex(const Sampler* sampler) const
    {
        if (sampler == nullptr || !sampler->IsValid())
            return -1;
    ...
    void RHIDeviceD3D12::Map(const GPUResource* resource, Mapping* mapping) const
    {
        auto internal_state = to_internal(resource);
    ...
    void RHIDeviceD3D12::Unmap(const GPUResource* resource) const
    {
        auto internal_state = to_internal(resource);
    ...
    void RHIDeviceD3D12::SetCommonSampler(const StaticSampler* sam)
    {
    ...
    CommandList RHIDeviceD3D12::BeginCommandList(RHIQueueType queue)
    {
        HRESULT hr;
        ...
        if (GetCommandList(cmd) == nullptr)
        {
            ...
            hr = device->CreateCommandList1(0, queues[(uint32_t)queue].desc.Type, D3D12_COMMAND_LIST_FLAG_NONE, IID_PPV_ARGS(&commandLists[cmd][(uint32_t)queue]));
            assert(SUCCEEDED(hr));

            std::wstringstream wss;
            wss << "cmd" << cmd;
            commandLists[cmd][(uint32_t)queue]->SetName(wss.str().c_str());

            descriptors[cmd].init(this);
        }

        // Start the command list in a default state:
        hr = GetFrameResources().commandAllocators[cmd][(uint32_t)queue]->Reset();
        assert(SUCCEEDED(hr));
        hr = GetCommandList(cmd)->Reset(GetFrameResources().commandAllocators[cmd][(uint32_t)queue].Get(), nullptr);
        assert(SUCCEEDED(hr));
        ...
        GetCommandList(cmd)->SetDescriptorHeaps(_countof(heaps), heaps);
    ...
    void RHIDeviceD3D12::SubmitCommandLists()
    {
        HRESULT hr;
        ...
        // Submit current frame:
        {
            ...
            for (CommandList cmd = 0; cmd < cmd_last; ++cmd)
            {
                ...
                if (submit_queue == RHIQueueType::Count)
                {
                ...
                if (meta.queue != submit_queue || !meta.waits.empty()) // new queue type or wait breaks submit batch
                {
                    ...
                    if (queues[(uint32_t)submit_queue].submit_count > 0)
                    {
                        queues[(uint32_t)submit_queue].queue->ExecuteCommandLists(
                            queues[(uint32_t)submit_queue].submit_count,
                            queues[(uint32_t)submit_queue].submit_cmds
                        );
                        queues[(uint32_t)submit_queue].submit_count = 0;
                    }

                    // signal status in case any future waits needed:
                    hr = queues[(uint32_t)submit_queue].queue->Signal(
                        queues[(uint32_t)submit_queue].fence.Get(),
                    ...
                    for (auto& wait : meta.waits)
                    {
                        ...
                        hr = queues[(uint32_t)submit_queue].queue->Wait(
                            queues[(uint32_t)wait_meta.queue].fence.Get(),
                ...
                assert(submit_queue < RHIQueueType::Count);
                queues[(uint32_t)submit_queue].submit_cmds[queues[(uint32_t)submit_queue].submit_count++] = GetCommandList(cmd);
            ...
            assert(submit_queue < RHIQueueType::Count);
            assert(queues[(uint32_t)submit_queue].submit_cmds > 0);
            queues[(uint32_t)submit_queue].queue->ExecuteCommandLists(
                queues[(uint32_t)submit_queue].submit_count,
                queues[(uint32_t)submit_queue].submit_cmds
            );
        ...
        // Begin next frame:
        {
            ...
            hr = GetGraphicsQueue()->Signal(descriptorheap_res.fence.Get(), descriptorheap_res.fenceValue);
            assert(SUCCEEDED(hr));
            ...
            hr = GetGraphicsQueue()->Signal(descriptorheap_sam.fence.Get(), descriptorheap_sam.fenceValue);
            assert(SUCCEEDED(hr));
    ...
    void RHIDeviceD3D12::WaitForGPU() const
    {
        ComPtr<ID3D12Fence> fence;
    ...
    void RHIDeviceD3D12::ClearPipelineStateCache()
    {
        allocationhandler->destroylocker.lock();
        ...
        for (uint32_t i = 0; i < _countof(pipelines_worker); ++i)
        {
    ...
    RHITexture RHIDeviceD3D12::GetBackBuffer(const SwapChain* swapchain) const
    {
    ...
    void RHIDeviceD3D12::WaitCommandList(CommandList cmd, CommandList wait_for)
    {
    ...
    void RHIDeviceD3D12::BindScissorRects(CommandList commandList, uint32_t scissorCount, const Rect* pScissorRects)
    {
        ...
        {
            pRects[i].left = LONG(pScissorRects[i].x);
            pRects[i].top = LONG(pScissorRects[i].y);
            pRects[i].right = LONG(pScissorRects[i].Right());
            pRects[i].bottom = LONG(pScissorRects[i].Bottom());
        }

        GetCommandList(commandList)->RSSetScissorRects(scissorCount, pRects);
    }

    void RHIDeviceD3D12::BindViewport(CommandList commandList, const Viewport& viewport)
    {
        GetCommandList(commandList)->RSSetViewports(1, (D3D12_VIEWPORT*)&viewport);
    }

    void RHIDeviceD3D12::BindViewports(CommandList commandList, uint32_t viewportCount, const Viewport* pViewports)
    {
    ...
    void RHIDeviceD3D12::BindSampler(CommandList commandList, uint32_t slot, const Sampler* sampler)
    {
        ALIMER_ASSERT(slot < GPU_SAMPLER_HEAP_COUNT);

        if (descriptors[commandList].SAM[slot] != sampler)
        {
            descriptors[commandList].SAM[slot] = sampler;
    ...
    void RHIDeviceD3D12::BindStencilRef(CommandList commandList, uint32_t value)
    {
        GetCommandList(commandList)->OMSetStencilRef(value);
    }

    void RHIDeviceD3D12::BindBlendFactor(float r, float g, float b, float a, CommandList cmd)
    {
    ...
    void RHIDeviceD3D12::UpdateBuffer(CommandList commandList, const GPUBuffer* buffer, const void* data, uint64_t size)
    {
        ...
        else
        {
            ...
            if (Any(buffer->desc.usage, BufferUsage::Uniform)
                || Any(buffer->desc.usage, BufferUsage::Vertex))
            {
                barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
            }
            else if (Any(buffer->desc.usage, BufferUsage::Index))
            {
    ...
    void RHIDeviceD3D12::QueryEnd(const GPUQueryHeap* heap, uint32_t index, CommandList cmd)
    {
        auto internal_state = to_internal(heap);
    ...
    void RHIDeviceD3D12::QueryResolve(const GPUQueryHeap* heap, uint32_t index, uint32_t count, CommandList cmd)
    {
        if (count == 0)
            return;
    ...
    void RHIDeviceD3D12::Barrier(const GPUBarrier* barriers, uint32_t numBarriers, CommandList cmd)
    {
    ...
    void RHIDeviceD3D12::BuildRaytracingAccelerationStructure(const RaytracingAccelerationStructure* dst, CommandList cmd, const RaytracingAccelerationStructure* src)
    {
        barrier_flush(cmd);
    ...
    void RHIDeviceD3D12::PushConstants(const void* data, uint32_t size, CommandList cmd)
    {
        std::memcpy(pushconstants[cmd].data, data, size);
        pushconstants[cmd].size = size;
    }

    GPUAllocation RHIDeviceD3D12::AllocateGPU(size_t dataSize, CommandList cmd)
    {
    ...
    void RHIDeviceD3D12::EventBegin(const StringView& name, CommandList cmd)
    {
        auto wName = ToUtf16(name);
        PIXBeginEvent(GetCommandList(cmd), PIX_COLOR_DEFAULT, wName.c_str());
    }

    void RHIDeviceD3D12::EventEnd(CommandList cmd)
    {
        PIXEndEvent(GetCommandList(cmd));
    }

    void RHIDeviceD3D12::SetMarker(const StringView& name, CommandList cmd)
    {
        auto wName = ToUtf16(name);
